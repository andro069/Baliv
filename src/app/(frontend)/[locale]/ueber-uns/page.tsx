...
const D = {
  ...
  werWirSindPunkte: [
    ...
    { title: 'Beratung in vier Sprachen', text: 'Persönlich, per E-Mail, Telefon oder WhatsApp.' },
...
async function lade(locale: Locale) {
  try {
    ...
    return (await payload.findGlobal({ slug: 'ueber-uns-page', locale, depth: 1 })) as any
  } catch {
    return null
  }
}
...
export async function generateMetadata({
  ...
  return seitenMetadata({ seite: 'ueber-uns', locale, title, description })
}
...
export default async function UeberUnsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const hero = {
    ...
    imageAlt: t(cms?.hero?.imageAlt, 'Baliv Residence in Bar'),
  }

  const wer = {
    eyebrow: t(cms?.werWirSind?.eyebrow, 'Wer wir sind'),
    headline: t(cms?.werWirSind?.headline, 'Inhabergeführt. Mehrsprachig. Direkt.'),
    text: t(
      cms?.werWirSind?.text,
  ...
  const bar = {
    ...
    linkHref: localizeHref(t(cms?.warumBar?.linkHref, '/lage'), locale),
    image: mediaUrl(cms?.warumBar?.image, '/stari-bar-altstadt.webp'),
    imageAlt: t(cms?.warumBar?.imageAlt, 'Stari Bar'),
  }
  ...
  const cta = {
    ...
    buttonLink: localizeHref(t(cms?.cta?.buttonLink, '/kontakt'), locale),
    note: t(cms?.cta?.note, 'In der Regel Antwort innerhalb von 24 Stunden · Beratung in vier Sprachen'),
  }
      ...
      <section className="pb-24 px-8 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="relative aspect-[4/3] overflow-hidden rounded">
            <Image src={bar.image} alt={bar.imageAlt} fill className="object-cover" />
          </div>
          <div>
