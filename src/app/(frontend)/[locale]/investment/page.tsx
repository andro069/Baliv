...
import { formatEuro, formatZahl, fuelle, text } from '@/i18n/format'
...
export async function generateMetadata({
  ...
  try {
    ...
    const cms = (await payload.findGlobal({ slug: 'investment-page', locale, depth: 0 })) as any
  ...
  return seitenMetadata({ seite: 'investment', locale, title, description })
}

// WhatsApp-Nummer ohne Plus; die vorbelegte Nachricht kommt je Sprache aus „Website & Sprachen“.
const WHATSAPP_NUMMER = '38268517873'
...
const defaultZeitleiste = [
  { year: '2026', event: 'Genehmigung · Baubeginn Oktober 2026', note: '' },
...
/**
 * Euro-Betrag ohne Nachkommastellen. Deutsch bleibt bei der bisherigen Schreibweise
 * „2.500 €“ mit normalem Leerzeichen (Intl setzt ein geschütztes Leerzeichen).
 */
const eurFuer = (locale: Locale) => (n: number) =>
  locale === defaultLocale ? `${Math.round(n).toLocaleString('de-DE')} €` : formatEuro(Math.round(n), locale)
...
export default async function InvestmentPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [cms, { ui }] = await Promise.all([
    payload.findGlobal({ slug: 'investment-page', locale }),
    getWebsite(locale),
  ])
  const eur = eurFuer(locale)
  const dec = (n: number) => formatZahl(n, locale, { maximumFractionDigits: 2 })
  const whatsappUrl = `https://wa.me/${WHATSAPP_NUMMER}?text=${encodeURIComponent(ui.whatsappNachricht)}`
  ...
  const heroEyebrow = text(hero.eyebrow, 'Investment')
  const heroImageAlt = text(hero.imageAlt, 'Baliv Residence Aussicht')
  ...
  const warumEyebrow = text(warum.eyebrow, 'Standortvorteil')
  const warumImageAlt = text(warum.imageAlt, 'Baliv Residence')
  ...
  // Texte der Rechnung: Vorlagen mit Platzhaltern, je Sprache im Backend gepflegt.
  const mietTexte = {
    eyebrow: text(r.eyebrow, 'Mietrendite'),
    labelHauptsaison: text(r.labelHauptsaison, 'Hauptsaison'),
    labelNebensaison: text(r.labelNebensaison, 'Nebensaison'),
    labelGesamt: text(r.labelGesamt, 'Vermietung gesamt'),
    labelVerwaltung: text(r.labelVerwaltung, 'Verwaltung und Reinigung'),
    vorlageSaison: text(r.vorlageSaison, '{wochen} Wochen · {preis}/Woche'),
    vorlageGesamt: text(r.vorlageGesamt, '{wochen} Wochen/Jahr'),
    vorlageVerwaltung: text(r.vorlageVerwaltung, '{quote} % der Einnahmen'),
    rechnerTitel: text(r.rechnerTitel, 'Beispielrechnung · {flaeche} m² Zweizimmerwohnung'),
    zeileKaufpreis: text(r.zeileKaufpreis, 'Kaufpreis ({flaeche} m² × {preisProQm})'),
    zeileHauptsaison: text(r.zeileHauptsaison, 'Hauptsaison · {wochen} Wochen × {preis}'),
    zeileNebensaison: text(r.zeileNebensaison, 'Nebensaison · {wochen} Wochen × {preis}'),
    zeileBrutto: text(r.zeileBrutto, 'Bruttoeinnahmen'),
    zeileVerwaltung: text(r.zeileVerwaltung, '– Verwaltung und Reinigung ({quote} %)'),
    zeileBetrieb: text(r.zeileBetrieb, '– Betrieb und Instandhaltung'),
    zeileVorSteuer: text(r.zeileVorSteuer, '= Ertrag vor Steuer'),
    zeileSteuer: text(r.zeileSteuer, '– Einkommensteuer {satz} %'),
    zeileNetto: text(r.zeileNetto, '= Nettoertrag'),
    zeileRendite: text(r.zeileRendite, 'Rendite auf {kaufpreis}'),
  }

  const rentalRows: { label: string; value: string; strong?: boolean }[] = [
    {
      label: fuelle(mietTexte.zeileKaufpreis, { flaeche: dec(size), preisProQm: eur(pricePerSqm) }),
      value: eur(kaufpreis),
    },
    {
      label: fuelle(mietTexte.zeileHauptsaison, { wochen: dec(hsWochen), preis: eur(hsPreis) }),
      value: eur(einnahmenHS),
    },
    {
      label: fuelle(mietTexte.zeileNebensaison, { wochen: dec(nsWochen), preis: eur(nsPreis) }),
      value: eur(einnahmenNS),
    },
    { label: mietTexte.zeileBrutto, value: eur(brutto), strong: true },
    { label: fuelle(mietTexte.zeileVerwaltung, { quote: dec(verwaltungQuote) }), value: `– ${eur(verwaltung)}` },
    { label: mietTexte.zeileBetrieb, value: `– ${eur(betrieb)}` },
    { label: mietTexte.zeileVorSteuer, value: eur(vorSteuer), strong: true },
    { label: fuelle(mietTexte.zeileSteuer, { satz: dec(steuersatz) }), value: `– ${eur(steuer)}` },
  ]

  const mietListe = [
    {
      label: mietTexte.labelHauptsaison,
      value: fuelle(mietTexte.vorlageSaison, { wochen: dec(hsWochen), preis: eur(hsPreis) }),
    },
    {
      label: mietTexte.labelNebensaison,
      value: fuelle(mietTexte.vorlageSaison, { wochen: dec(nsWochen), preis: eur(nsPreis) }),
    },
    { label: mietTexte.labelGesamt, value: fuelle(mietTexte.vorlageGesamt, { wochen: dec(hsWochen + nsWochen) }) },
    { label: mietTexte.labelVerwaltung, value: fuelle(mietTexte.vorlageVerwaltung, { quote: dec(verwaltungQuote) }) },
  ]
  const renditeText = formatZahl(rendite, locale, { minimumFractionDigits: 1, maximumFractionDigits: 1 })

  // ── Steuern ──
  const st = (cms as any)?.steuerSektion ?? {}
  const steuerSektion = {
    eyebrow: text(st.eyebrow, 'Steuern'),
  ...
  // Goldene Hervorhebung: bevorzugt über das Häkchen „hervorheben“. Solange in keiner
  // Zeile ein Häkchen gesetzt ist, gilt die bisherige Erkennung über den deutschen Wert.
  const alteHervorhebung = (value: string) => value === '0 %' || value === 'entfällt'
  const hatHaekchen = cmsTax.some((t: any) => t?.hervorheben === true)
  const taxAdvantages: { label: string; value: string; note: string; hervorheben: boolean }[] = cmsTax.length > 0
    ? cmsTax.map((t: any) => {
        const value = t.value ?? ''
        return {
          label: t.label ?? '',
          value,
          note: t.note ?? '',
          hervorheben: hatHaekchen ? t.hervorheben === true : alteHervorhebung(value),
        }
      })
    : defaultTaxAdvantages.map((t) => ({ ...t, hervorheben: alteHervorhebung(t.value) }))
  ...
  const cta = {
    ...
    tags:
      (c.tags ?? []).length > 0
        ...
        : ['In der Regel Antwort innerhalb von 24 Stunden', 'Beratung in vier Sprachen', 'Kein Makler', 'Direkt vom Bauträger'],
  }
    ...
    <main className="bg-[#F0EDE8]" style={{ fontFamily: 'var(--font-raleway), sans-serif' }}>
      ...
      <section className="py-24 px-8 md:px-16 lg:px-24 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <p className="text-[#B69252] text-xs tracking-[0.3em] uppercase mb-4">{warumEyebrow}</p>
            <h2
      ...
      <section className="py-24 px-8 md:px-16 lg:px-24 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div>
            <p className="text-[#B69252] text-xs tracking-[0.3em] uppercase mb-4">{mietTexte.eyebrow}</p>
            <h2
              className="text-[#151E39] text-3xl md:text-5xl leading-tight mb-6"
              style={{ fontFamily: 'var(--font-playfair), serif' }}
            >
          ...
          <div className="bg-[#151E39] rounded p-8">
            <p className="text-[#B69252] text-xs tracking-widest uppercase mb-6">
              {fuelle(mietTexte.rechnerTitel, { flaeche: dec(size) })}
            </p>
            ...
            <div className="bg-[#B69252]/10 border border-[#B69252]/30 rounded p-5 mb-6">
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[#B69252] text-xs tracking-widest uppercase mb-1">{mietTexte.zeileNetto}</p>
                  <p className="text-white text-3xl font-light" style={{ fontFamily: 'var(--font-playfair), serif' }}>
                    {eur(netto)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[#B69252] text-xs tracking-widest uppercase mb-1">{fuelle(mietTexte.zeileRendite, { kaufpreis: eur(kaufpreis) })}</p>
      ...
      <section className="bg-[#151E39] py-24 px-8 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <p className="text-[#B69252] text-xs tracking-[0.3em] uppercase mb-4">{steuerSektion.eyebrow}</p>
              <h2
              ...
              <div className="space-y-3">
                {taxAdvantages.map((tax) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-xl font-light ml-6 flex-shrink-0 ${tax.hervorheben ? 'text-[#B69252]' : 'text-white'}`}
                      style={{ fontFamily: 'var(--font-playfair), serif' }}
                    >
                      {tax.value}
                    </div>
