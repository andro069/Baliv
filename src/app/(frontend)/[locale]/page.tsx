...
import { text as t } from '@/i18n/format'
...
export async function generateMetadata({
  ...
  try {
    ...
    const cms = (await payload.findGlobal({ slug: 'homepage', locale, depth: 0 })) as any
  ...
  return seitenMetadata({ seite: '', locale, title, description })
}

export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  const locale = await seitenLocale(params)
  const payload = await getPayload({ config })
  const [cmsRaw, footerCms, { ui }] = await Promise.all([
  ...
  const heroButtons = [
    {
      ...
      href: localizeHref(t(cms?.hero?.primaryButtonLink, '/kontakt'), locale),
      primary: true,
    },
    {
      label: t(cms?.hero?.secondaryButtonLabel, 'Projekt entdecken'),
      href: localizeHref(t(cms?.hero?.secondaryButtonLink, '/architektur'), locale),
      primary: false,
    },
  ]
  const heroSlides =
  ...
  // ── Lage ─────────────────────────────────────────────────────────────
  const lageEyebrow = t(cms?.lage?.eyebrow, 'Die Lage')
  const lageAlt1 = t(cms?.lage?.imageAlt1, 'Stari Bar Festung')
  const lageAlt2 = t(cms?.lage?.imageAlt2, 'Olivenhaine')
  const lageAlt3 = t(cms?.lage?.imageAlt3, 'Hafen von Bar')
  ...
  // ── Wohnungen ────────────────────────────────────────────────────────
  const wohnungenEyebrow = t(cms?.wohnungen?.eyebrow, 'Die Wohnungen')
  const wohnungenDetails = t(cms?.wohnungen?.detailsLabel, 'Details')
  ...
  // ── Investment ───────────────────────────────────────────────────────
  const investEyebrow = t(cms?.investment?.eyebrow, 'Investment')
  const investLinkLabel = t(cms?.investment?.linkLabel, 'Vollständige Investment-Analyse')
  const investImageAlt = t(cms?.investment?.imageAlt, 'Bar Hafen')
  ...
  // ── CTA ──────────────────────────────────────────────────────────────
  const ctaButtonLabel = t(cms?.cta?.buttonLabel, 'Exposé anfragen')
  const ctaImageAlt = t(cms?.cta?.imageAlt, 'Terrasse')
  ...
  // Rechtslinks werden deutsch gepflegt und hier in die Sprachadresse übersetzt.
  ...
  ).map((item) => ({ label: item.label, href: localizeHref(item.href, locale) }))
    ...
    <main className="bg-[#F0EDE8]" style={{ fontFamily: 'var(--font-raleway), sans-serif' }}>
      ...
      <section className="py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-8 md:px-16 grid md:grid-cols-2 gap-16 items-center">
          <div>
            <p className="text-[#B69252] text-xs tracking-[0.3em] uppercase mb-4">{lageEyebrow}</p>
            <h2
      ...
      <section className="py-24 bg-[#151E39]">
        <div className="max-w-7xl mx-auto px-8 md:px-16">
          <div className="text-center mb-16">
            <p className="text-[#B69252] text-xs tracking-[0.3em] uppercase mb-4">{wohnungenEyebrow}</p>
            <h2
      ...
      <section className="py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-8 md:px-16 grid md:grid-cols-2 gap-16 items-center">
          ...
          <div>
            <p className="text-[#B69252] text-xs tracking-[0.3em] uppercase mb-4">{investEyebrow}</p>
            <h2
              className="text-[#151E39] text-4xl md:text-5xl leading-tight mb-8"
              style={{ fontFamily: 'var(--font-playfair), serif' }}
            >
            ...
            <Link
              href={pathFor('investment', locale)}
      ...
      <section id="kontakt" className="relative py-32 overflow-hidden">
        <div className="absolute inset-0">
          <Image src="/terrasse-berge.webp" alt={ctaImageAlt} fill className="object-cover" />
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a
              href={`https://wa.me/${telefonZiffern}?text=${encodeURIComponent(ui.whatsappNachricht)}`}
              target="_blank"
