...
export type KontaktFormContent = {
  ...
  /** Wort im Einwilligungstext, das auf die Datenschutzerklärung verlinkt — je Sprache gepflegt. */
  datenschutzLinkWort: string
  fehlerText: string
...
const defaultContent: KontaktFormContent = {
  ...
  exposeCheckboxText: 'Grundrisse, Preisliste & Baubeschreibung — per E-Mail',
  datenschutzText:
  ...
  datenschutzLinkWort: 'Datenschutzerklärung',
  fehlerText:
...
/**
 * Stellt das im Backend je Sprache gepflegte Link-Wort (DE „Datenschutzerklärung")
 * im Label als Link auf die Datenschutzseite der jeweiligen Sprache dar, damit der
 * Text selbst im Backend frei editierbar bleibt.
 */
function mitDatenschutzLink(text: string, wort: string, href: string): React.ReactNode {
  if (!wort) return text
  const teile = text.split(wort)
  if (teile.length === 1) return text
  return teile.map((teil, i) => (
    <React.Fragment key={i}>
      {teil}
      {i < teile.length - 1 && (
        <Link href={href} className="text-[#B69252] underline hover:text-[#a07d3f]">
          {wort}
        </Link>
      )}
    </React.Fragment>
...
export function KontaktForm({ content }: { content?: Partial<KontaktFormContent> }) {
  ...
  const datenschutzHref = pathFor('datenschutz', locale)
  const datenschutzLink = (text: string) => mitDatenschutzLink(text, c.datenschutzLinkWort, datenschutzHref)
    ...
    try {
      const res = await fetch('/api/kontakt', {
        ...
        body: JSON.stringify({
          ...
          // Sprache der Seite: bestimmt Fehlermeldungen, Autoresponder und Exposé-Anhang.
          locale,
        }),
      })
    ...
    if (f.blockType === 'checkbox') {
      return (
        <label key={key} className="flex items-start gap-3 cursor-pointer">
          ...
          />
          <span className="text-[#151E39]/60 text-sm leading-relaxed">{datenschutzLink(label)}</span>
        </label>
      )
    }
      ...
      <div key={key}>
        ...
        {f.blockType === 'select' && (
          ...
          (f.options ?? []).length > 0 && (f.options ?? []).length <= 4 ? (
            ...
            >
              <option value="">{ui.bitteWaehlen}</option>
              {(f.options ?? []).map((opt) => (
  ...
  return (
    ...
    >
      ...
      {!hatDatenschutzCheckbox && c.datenschutzText && (
        <p className="text-[#151E39]/30 text-xs leading-relaxed">{datenschutzLink(c.datenschutzText)}</p>
      )}
