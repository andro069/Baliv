...
export async function generateMetadata({
  ...
  try {
    ...
    const cms = (await payload.findGlobal({ slug: 'architektur-page', locale, depth: 0 })) as any
  ...
  return seitenMetadata({ seite: 'architektur', locale, title, description })
}
...
export default async function ArchitekturPage({ params }: { params: Promise<{ locale: string }> }) {
  const locale = await seitenLocale(params)
  const payload = await getPayload({ config })
  const cms = await payload.findGlobal({ slug: 'architektur-page', locale })

  const hero = (cms as any)?.hero ?? {}
  const heroImageAlt = text(hero.imageAlt, 'Blick vom Penthouse durch den Rundbogen über Bar und die Adria')
  ...
  const prinzipienImageAlt = text(prinzipien.imageAlt, 'Fassadendetail Baliv Residence')
  ...
  const fassadeImageAlt = text(fassade.imageAlt, 'Baliv Residence Gebäudeansicht')
  ...
  const materialienImageAlt = text(materialienGroup.imageAlt, 'Materialpalette Baliv Residence')
  ...
  const ctaButtonSecondaryLink = localizeHref(cta.buttonSecondaryLink ?? '/wohnungen', locale)
