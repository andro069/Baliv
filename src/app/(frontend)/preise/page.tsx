      ...
      <section className="bg-[#151E39] pt-32 pb-20 px-8 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
            {[
              { v: `${statPrefix} ${eur(types[0]?.examplePrice ?? 75800)} €`, l: types[0]?.type ?? 'Studio' },
              { v: `${statPrefix} ${eur(types[1]?.examplePrice ?? 116975)} €`, l: types[1]?.type ?? 'Zweizimmer' },
              { v: `${statPrefix} ${eur(types[2]?.examplePrice ?? 185800)} €`, l: types[2]?.type ?? 'Penthouse-Ebene' },
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="rounded p-4 mb-5 bg-white/5 border border-white/10">
                    <div className="text-xs tracking-widest uppercase mb-1 text-white/30">
                      {typesSection.exampleLabel ?? 'Beispiel'} · {area(t.exampleSize)} m²
                    </div>
                    <div className="text-2xl font-light text-white" style={{ fontFamily: 'var(--font-playfair), serif' }}>
                      {approx(c.priceApprox)}{eur(t.examplePrice)} €
                    </div>
                ...
                <div key={t.nr} className="bg-white/5 border border-white/10 rounded p-7">
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white">{area(t.exampleSize)} m²</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-white/40">{beispielSection.rowPricePerSqm ?? 'Preis/m²'}</span>
                      <span className="text-white">{eur(t.examplePricePerSqm)} €/m²</span>
                    </div>
      ...
      <section id="kontakt" className="py-24 px-8 md:px-16 lg:px-24">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-[#151E39]/30 text-xs mt-6">
            {cta.note ?? 'In der Regel Antwort innerhalb von 24 Stunden · Deutschsprachig · Direkt vom Bauträger · Kein Makler'}
          </p>
        </div>
      </section>
